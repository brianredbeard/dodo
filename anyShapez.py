...
class AnyThing(pypeType): #object):
  ...
  def __init__(self, obj,name="valve",fileName='ballDN15.stp',ports='0:0:0'):
    ...
    obj.addProperty("App::PropertyString","FileName","AnyThing","The file of the shape (inside ./shapez)").FileName=fileName
    portslist=list()
    if ports:
    ...
    if fileName:
      s=Part.Shape()
      path=join(dirname(abspath(__file__)),"shapez",fileName)
      if exists(path):
        s.read(path)
        obj.Shape=s
      else:
